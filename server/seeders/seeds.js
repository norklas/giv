...
const { User, Cause, Category, Comment, Medal, Point, Share } = require('../models');
...
db.once('open', async () => {
  ...
  for (let i = 0; i < 100; i++) {
    const title = faker.company.catch_phrase();
    const description = faker.lorem.paragraph();
    const url = faker.internet.url();
    const category = createCategories[Math.floor(Math.random() * createdCategories.ops.length)];
    const location = faker.address();
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const causeNum = Math.floor(Math.random) * createdCategories.ops.length;
    const causes = [];
    for (let j = 0; j < causeNum; j++) {
      const randomCauseIndex = Math.floor(Math.random() * createdCauses.ops.length);
